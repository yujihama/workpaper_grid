...
| 列幅・行高 | `getColumnWidth` / `getRowHeight` | 列幅は 60〜320px に丸め。行高は 30px 未満（既定相当）なら折り返しが読める 34px、それ以上はそのまま |
...
- 行高: 行番号の境界（上下 4px）をドラッグ（Excel 同様。glide に無いので自前実装。カーソルが ↕ になる位置でつかむ）。自分で変えた行はその高さで表示し、それ以外で 30px 未満の行は既定扱いで 34px に補正
...
## 未対応・要検討
- 結合セルの表示（エンジンは保持するが、wasm APIから結合範囲を取る手段が未確認。表示側で別途保持する設計が必要）
- 全セル編集可（数式含む）。編集させたくないセル（見出し・計算ロジック）の保護はテンプレート登録時の指定で行う想定
- 行の高さ・セル内改行の表示調整、列幅のドラッグ変更の保存
- コメントはブラウザ内のみ（xlsxのメモとしては書き出していない）
- 図形・画像・マクロ・ピボットの保持は未検証
- サーバのセッションはメモリ保持（本番は永続化と排他制御が必要）
- `/api/ai` はダミー。実運用では既存AIロジックの出力を ops 形式に整形する
