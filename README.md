...
| 枠固定（freeze panes） | ○ | 残る | 列の固定のみ反映（行の固定は glide に無い） |
| 条件付き書式 | ○ | 残る | 塗り・文字色は反映。アイコンは記号、データバーは棒で近似 |
| 名前定義 | ○ | 残る | — |
| Excel のメモ（旧コメント） | × IronCalc 単独では開けない → **メモを除いたコピーを渡して開く** | 元のパーツを温存 | 紫の三角＋選択で本文（読み取り専用） |
| グラフ・図形・画像 | ○ | IronCalc 単独では消える → **元 xlsx を土台にした保存で温存** | 画像は実物、グラフ/図形は枠と名前 |
| オートフィルタ | ○ | 同上で温存（行列操作に追従） | 非表示（フィルタ操作は未対応） |
| 入力規則（ドロップダウン等） | ○ | 同上で温存 | list は ▾ で候補選択 |
| ハイパーリンク | ○ | 同上で温存 | 青下線、Ctrl+クリックで開く |
| マクロ・ピボット | 未検証 | 差し替え保存ならパーツは残る見込み（未検証） | — |

IronCalc 単独では図形・メモ等が失われるため、保存は「元の xlsx を土台に、IronCalc の出力からセル部分だけを移植する」方式にした（「元ファイルの温存」参照）。
...
- Excel のメモ付きブック: メモを除いたコピーを IronCalc に渡すことで開ける。メモは元ファイルから読んで表示（「元ファイルの温存」参照）
- グラフ・図形・画像・オートフィルタ・入力規則・ハイパーリンク: 元 xlsx を土台にセル部分だけを移植して保存するので残る。画像は実物、グラフ/図形は枠、リンクと入力規則は操作可
- 行高・列幅の寸法: IronCalc の単位を Excel ピクセルに換算し、未指定の行は Excel と同じく自動調整（`units.ts`）

## 未対応・要検討
- **編集ロックの永続化**: IronCalc に保護属性が無いため xlsx には残らない。テンプレート側で入力可セルを定義して、開くときにロックを復元する設計が必要
- **AI 向けコメントはブラウザ内のみ**: 指示として使い切る前提で、xlsx のメモとしては書き出していない。再生成後は消える
- **Excel のメモ**: 表示は既存のもののみ。新規作成・本文編集の書き戻しは未対応（保存時は元のパーツをそのまま残す）
- **グラフの描画**（枠と名前のみ）、画像・図形の移動やサイズ変更、入力規則の list 以外（数値範囲など）の検証
- **Excel 本体での保存ファイルの確認**: 「修復」ダイアログが出ないことは未確認。実際の調書で確認が必要
- **列幅の換算は Calibri 11pt 前提**: 既定フォントが異なるブックでは数 px ずれる。`styles.xml` の既定フォントから係数を出す拡張が必要
- マクロ（xlsm）・ピボットの温存は未検証（差し替え保存ならパーツは残る見込みだが、実ファイルで未確認）
